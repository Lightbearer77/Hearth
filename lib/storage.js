...
export const saveEvent = async (event) => {
  ...
  const now = Date.now();
  await db.runAsync(
    `INSERT INTO events (id, title, description, location, date, end_date, start_time, end_time, all_day, category_id, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    [
      ...
      event.allDay ? 1 : 0, event.categoryId, event.createdAt || now, now,
    ]
  );
};
