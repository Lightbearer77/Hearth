...
export const expandOccurrences = (event, rangeStartISO, rangeEndISO) => {
  ...
  // Series end date: governs occurrence START dates (a multi-day span
  // starting on `until` still runs its full duration — same semantics as
  // exdates). Clamping the range end covers every mode branch below.
  const until = event.recurrenceUntil || '';
  if (until) {
    if (until < event.date) return [];
    if (until < rangeEndISO) rangeEndISO = until;
  }
...
// ─── Split a series at a pivot occurrence ("this and all future") ───
// Pure. Returns:
//   truncatedMaster — the original master ending the day before `pivotIso`
//                     (exdates < pivot kept), or NULL when the pivot is the
//                     first occurrence — the caller deletes the master
//                     instead, since an empty series has no reason to live.
//   newMaster       — a clone starting at `pivotIso` under `newId`: same
//                     recurrence, interval, and (inherited) recurrenceUntil,
//                     duration preserved, exdates ≥ pivot migrated.
export const splitSeriesAt = (master, pivotIso, newId) => {
  const ex = Array.isArray(master.exdates) ? master.exdates : [];
  const dur = durationDays(master);
  const newMaster = {
    ...master,
    id: newId,
    date: pivotIso,
    endDate: dur > 0 ? addDaysISO(pivotIso, dur) : '',
    exdates: ex.filter(x => x >= pivotIso),
    createdAt: Date.now(),
  };
  if (pivotIso <= master.date) {
    return { truncatedMaster: null, newMaster };
  }
  const truncatedMaster = {
    ...master,
    recurrenceUntil: addDaysISO(pivotIso, -1),
    exdates: ex.filter(x => x < pivotIso),
  };
  return { truncatedMaster, newMaster };
};
