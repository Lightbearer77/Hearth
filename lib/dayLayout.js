...
// Order plain events for a single day: all-day first, then timed by start
// time (chronological), deterministic title tiebreak. Shared by DayDetail
// and AgendaView so every day-scoped list orders identically. Non-mutating.
export const sortEventsByTime = (events) =>
  events.slice().sort((a, b) => {
    const aAll = a.allDay ? 0 : 1;
    const bAll = b.allDay ? 0 : 1;
    if (aAll !== bAll) return aAll - bAll;
    const am = startMinutesOf(a);
    const bm = startMinutesOf(b);
